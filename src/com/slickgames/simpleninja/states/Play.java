...
public class Play extends GameState {
		...
		if (!MyInput.isDown(MyInput.BLOCK)) {
			...
			if (MyInput.isPressed(MyInput.ATTACK) && !swinging) {
				if (cl.isPlayerOnGround()) {
					...
					player.getAnimation().setSpeed(player.stamina > 0 ? 1 / (42f + swingSpeed) : 1 / (100f));
		...
		for (Body b : bodies) {
			...
			try {
				if (enemies.contains((Enemy) b.getUserData(), true)) {
					...
					// cl.enemiesShot.clear(); Comment out if you notice
					// problems with "ghost" enemies
				}
			} catch (java.lang.ClassCastException e) {
			}
			world.destroyBody(b);
	...
	public void render() {
		...
		sr.box(b2dCam.position.x - 1.66f, b2dCam.position.y + .80f, 0, (float) (.0025f * player.stamina), .025f, 0);
